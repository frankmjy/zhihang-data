...
export function getAppId(path: string): string | null | undefined {
  ...
  const legacyAppIdKey = ['MIAO', 'DA_APP_ID'].join('');
  const legacyAppId = (window as unknown as Record<string, string | undefined>)[
    legacyAppIdKey
  ];

  if (window.ZHIHANG_SYNC_APP_ID) {
    return window.ZHIHANG_SYNC_APP_ID;
  }
